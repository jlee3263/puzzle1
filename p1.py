...
class Game():
    ...
    def is_comply(self, future_row, future_col, future_face_up_value):
        ...
        print(f"    future grid score label:{future_grid_score_label}, future score:{future_score}, future row and col:{future_row}, {future_col}")  
    ...
    def revert_back_one_step(self, undo_direction):
        self.score -= self.dice.faces[self.dice.current_face_up_index]
        print("Deducting back the score to prev: {self.score}") 

        self.dice.fill_in_one_value_for_one_face(False, self.dice.current_face_up_index)
    ...
    def find_one_suitable_value(self, future_row, future_col):
    ...
    def get_tip_back_direction(self, future_direction):
        ...
        if future_direction == RIGHT:
            undo_direction = LEFT
        print("direction just made: {future_direction}, undo direction: {undo_direction}")
...
if __name__ == "__main__":
    ...
    # start a new game and new dice
    my_game = Game(GRID, Dice([False] * 6))
    my_game.print_game_state()
    my_game.play()
